 ...
 d3.csv("http://elsiehsieh.github.io/disastermap/disaster.csv", function(data){
    ...
    for (var i=0; i< data.length; i++){
      ...
      for(var j=0; j<town.objects.sector.geometries.length;j++){
        ...
        if(dataTown==jsonTown){
          town.objects.sector.geometries[j].properties.traffic = dataValue
          break;
        }
  ...
  svg.append("g")
    ...
    .style("fill", function(d) {
     var Traffic = d.properties.traffic;
      if (Traffic) {
    ...
    .on("mouseover", function(d) {
      ...
      .style("opacity", 1)
      div.text(d.properties.T_Name+ " ─ 交通阻斷數量：" + d.properties.traffic) //+ " : " + rateById[d.TOWN_ID] d.properties.T_Name
